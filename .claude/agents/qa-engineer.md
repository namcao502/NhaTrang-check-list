...
- Mock localStorage via the existing `localStorageMock` in `jest.setup.ts`
...
5. Commit test files:
   ```bash
   cd C:/TEST/beach-check-list && git add <test files> && git commit -m "test: <ticket_id> <short description>"
   ```
