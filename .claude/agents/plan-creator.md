...
Next.js 15 App Router + TypeScript + Tailwind CSS, located at `C:/TEST/beach-check-list`.

Key files:
- `lib/types.ts` — `Item` and `Category` interfaces
- `lib/defaultData.ts` — pre-populated checklist data
- `lib/useChecklist.ts` — all state logic + localStorage persistence
- `app/page.tsx` — root page
- `components/` — UI components
- `__tests__/` — Jest test suite
- `tailwind.config.ts` — custom color tokens (`ocean-*`, `sand-*`, `coral-*`)
...
1. Read `C:/TEST/beach-check-list/CLAUDE.md` for architecture and conventions.
...
4. Flag any type changes in `lib/types.ts` that would cascade to other files.
...
|------|--------|-------------|
| `lib/types.ts` | Modify | Add X field to Item interface |
